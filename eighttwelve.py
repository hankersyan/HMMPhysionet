...
def main():
    '''
    Runs the HMM model on Icu types (1,2) and (3,4) and reports on the success of this way of aggregating the results.
    '''
                ...
                for model in models:
                    for resolution in resolutions:
                        for icutype in icutypes:
                            ...
                            name += "icutype="
                            listofalldicts = [heartratedict,WBCdict,tempdict,GCSdict,glucosedict,NIDSdict,urinedict]
                            # getting the general split betweeen train and test and extracting and imputing the inputs for all hmm inputs 
                            resolutions = [resolution] * 7
                            (validpatientsindices,realfeatmtrxtrain1,realfeatmtrxtest1,KNNfeats,reallos1,inputHmmallVars,ovlapinputHmmallVars,trainindices,testindices) = generatetraintestsplit(listofalldicts,wholefeat,los,8,ages,genders,urinedict,paticutypedictindex,resolutions)
                            dictindices= range(7)
                            ...
                            outputfeatstest2 = outputfeats[testindices,:]     
                            

                            hmm = True
                            hmmmsescore1 = Linearregr(traininghmmfeats1, testhmmfeats1, ytrain1, ytest1,hmm,log,numofstates,resolution,8,model)
                            hmmmsescore2 = Linearregr(traininghmmfeats2, testhmmfeats2, ytrain2, ytest2,hmm,log,numofstates,resolution,12,model)
                            hmm = False    
                            ...
                            success1 = int(hmmmsescore1 < baselinescore1)
                            success2 = int(hmmmsescore2 < baselinescore2) 
                            sucess = success1 and success2  
                            success1s = int(hmmmsescore1< sapsscore1)
                            success2s = int(hmmmsescore2< sapsscore2)
                            sucesss = success1s and success2s 
                            success1o = int(hmmmsescore1< alloutputscore1)
                            success2o = int(hmmmsescore2< alloutputscore2)
                            sucesso = success1o and success2o 
                            w.writerow([model,name ,over,log, numofstates,resolution,"12 34 ",hmmmsescore1 ,baselinescore1,alloutputscore1,sapsscore1,hmmmsescore2, baselinescore2,alloutputscore2,sapsscore2,success1,success2,sucess,success1s,success2s,sucesss,success1o,success2o,sucesso])
...
def hmmcompactgrid(nstate,inputtrain,trainlengths,ninits,dimensionality):
    ...
    '''
    covartypes = ['diag','tied','spherical']
    algorithms = ["viterbi","map"]
    selalg = algorithms[0]
    selcovartype = covartypes[0]
    bestscore = float("-inf")
    ...
    return (score,selalg,selcovartype)
